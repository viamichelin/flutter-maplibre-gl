...
final class MapLibreMapController
  ...
  /**
   * This container is returned as the final platform view instead of returning
   * `mapView`.
    ...
    switch (call.method) {
      ...
      case "map#toScreenLocationBatch": {
        ...
        for (int i = 0; i < param.length; i += 2) {
          PointF pointf = mapLibreMap.getProjection().toScreenLocation(new LatLng(param[i], param[i + 1]));
          reply[i] = pointf.x;
          reply[i + 1] = pointf.y;
        }
      ...
      case "fillExtrusionLayer#add": {
        ...
        final String filter = call.argument("filter");
        final boolean enableInteraction = call.argument("enableInteraction");
        final PropertyValue[] properties = LayerPropertyConverter.interpretFillExtrusionLayerProperties(
   ...
   * that {@link TextureView#onDetachedFromWindowInternal()} is called which
   * releases the
   * underlying surface.
   * This is required due to an FlutterEngine change that was introduce when
   * updating from
   * Flutter 2.10.5 to Flutter 3.10.0.
   * This FlutterEngine change is not calling `removeView` on a PlatformView which
   * causes the issue.
   * <p>
   * For more information check out:
   * <a href="https://github.com/flutter/flutter/issues/107297">Flutter issue</a>
   * <a href=
   * "https://github.com/flutter/engine/commit/8dc7cd1b1a33b5da561ac859cdcc49705ad1e598">Flutter
   * Engine commit that introduced the issue</a>
   * <a href="https://github.com/maplibre/flutter-maplibre-gl/issues/182">The
   * reported issue in the MapLibre repo</a>
   */
  ...
  private void updateMyLocationTrackingMode() {
    int[] mapboxTrackingModes = new int[] {
  ...
  private Bitmap getScaledImage(String imageId, float density) {
    ...
    // would be chosen.
    // For a device pixel ratio of 2.7, the asset .../3.0x/my_icon.png would be
    // chosen."
    // Source:
  ...
  boolean onMoveBegin(MoveGestureDetector detector) {
    // onMoveBegin gets called even during a move - move end is also not called
    // unless this function
    // returns
    // true at least once. To avoid redundant queries only check for feature if the
    // previous event
    // was ACTION_DOWN
